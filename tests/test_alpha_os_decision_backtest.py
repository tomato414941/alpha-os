...
def test_run_decision_backtest_replays_signal_into_equity_curve():
    ...
    result = run_decision_backtest(
        DecisionBacktestInput(
            ...
            subject_set_id="singleton:residual_return_3d",
    ...
    assert result.subject_set_id == "singleton:residual_return_3d"
    ...
    assert second_step.target_weight == pytest.approx(0.4142857143)
    assert second_step.target_notional == pytest.approx(0.435)
    assert result.steps[1].gross_leverage_exposure == pytest.approx(0.4142857143)
    assert result.steps[1].net_leverage_exposure == pytest.approx(0.4142857143)
    assert result.steps[1].long_leverage_exposure == pytest.approx(0.4142857143)
    assert result.steps[1].short_leverage_exposure == pytest.approx(0.0)
    assert result.gross_return_total == pytest.approx(0.137)
    assert result.net_return_total == pytest.approx(0.137)
    assert result.mean_traded_notional == pytest.approx(0.3075)
    assert result.mean_gross_leverage_exposure == pytest.approx(0.4571428571)
    assert result.mean_net_leverage_exposure == pytest.approx(0.4571428571)
    assert result.mean_long_leverage_exposure == pytest.approx(0.4571428571)
    assert result.mean_short_leverage_exposure == pytest.approx(0.0)
    assert result.mean_gross_notional_exposure == pytest.approx(0.4675)
    assert result.mean_long_notional_exposure == pytest.approx(0.4675)
...
def test_run_decision_backtest_reports_leverage_separately_from_notional():
    ...
    result = run_decision_backtest(
        DecisionBacktestInput(
            portfolio_id="paper_core",
            subject_set_id="singleton:carry",
            target_id="carry",
...
def test_run_decision_backtest_respects_no_trade_band_and_turnover_cost():
    ...
    assert result.steps[0].traded_notional == pytest.approx(0.4166666667)
    assert result.steps[0].cost == pytest.approx(0.0416666667)
    assert result.steps[0].cost_notional == pytest.approx(0.0416666667)
    assert second_step.target_weight == pytest.approx(0.4583333333)
    assert second_step.position_delta == pytest.approx(0.0)
    assert result.steps[1].turnover == pytest.approx(0.0)
...
def test_run_decision_backtest_tracks_drawdown_and_risk_scaling():
    ...
    assert second_step.target_weight == pytest.approx(0.85)
    assert result.max_drawdown == pytest.approx(0.17)
