...
def test_run_decision_backtest_replays_signal_into_equity_curve():
    ...
    assert second_step.target_weight == pytest.approx(0.3595238095)
    assert second_step.target_notional == pytest.approx(0.3775)
    assert result.steps[1].gross_leverage_exposure == pytest.approx(0.3595238095)
    assert result.steps[1].net_leverage_exposure == pytest.approx(0.3595238095)
    assert result.steps[1].long_leverage_exposure == pytest.approx(0.3595238095)
    assert result.steps[1].short_leverage_exposure == pytest.approx(0.0)
    assert result.gross_return_total == pytest.approx(0.1255)
    assert result.net_return_total == pytest.approx(0.1255)
    assert result.mean_traded_notional == pytest.approx(0.33625)
    assert result.mean_gross_leverage_exposure == pytest.approx(0.4297619048)
    assert result.mean_net_leverage_exposure == pytest.approx(0.4297619048)
    assert result.mean_long_leverage_exposure == pytest.approx(0.4297619048)
    assert result.mean_short_leverage_exposure == pytest.approx(0.0)
    assert result.mean_gross_notional_exposure == pytest.approx(0.43875)
    assert result.mean_long_notional_exposure == pytest.approx(0.43875)
...
def test_run_decision_backtest_respects_no_trade_band_and_turnover_cost():
    ...
    assert second_step.target_weight == pytest.approx(0.55)
...
def test_run_decision_backtest_tracks_drawdown_and_risk_scaling():
    ...
    assert second_step.target_weight == pytest.approx(0.7)
    assert result.max_drawdown == pytest.approx(0.14)
