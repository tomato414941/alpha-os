...
def test_admission_section_loads_from_toml(tmp_path):
    p = tmp_path / "cfg.toml"
    p.write_text(
        "\n".join(
            [
...
def test_config_runtime_helpers_follow_current_settings():
    ...
    cfg.lifecycle.live_proven_quality_min = 0.05
    ...
    assert cfg.lifecycle.live_proven_quality_min == pytest.approx(0.05)
