...
def test_run_cycle_accepts_json_fixture(tmp_path, capsys):
    ...
    _register_signal(main, db_path, "hyp_fixture")
    capsys.readouterr()

    rc = main(["debug-apply-evaluation", "--db", str(db_path), "--input", str(fixture_path)])
    assert rc == 0

    output = capsys.readouterr().out
    assert "Evaluation [created] BTC:residual_return_3d:2026-03-27" in output
    assert "Signal:   hyp_fixture" in output

    conn = sqlite3.connect(db_path)
    try:
        row = conn.execute(
            "SELECT signal_id, prediction_value, observation_value FROM evaluation_snapshots"
        ).fetchone()
...
def test_run_cycles_applies_multiple_fixture_days(tmp_path, capsys):
    ...
    rc = main(["debug-apply-evaluations", "--db", str(db_path), "--input", str(fixture_path)])
    assert rc == 0
...
def test_run_cycle_rejects_non_v1_asset_in_fixture(tmp_path):
    ...
    try:
        main(["debug-apply-evaluation", "--db", str(db_path), "--input", str(bad_fixture)])
...
def test_run_cycle_accepts_non_default_target_in_fixture(tmp_path, capsys):
    ...
    _register_signal(main, db_path, "hyp_fixture", target_id="residual_return_1d")
    capsys.readouterr()

    assert main(["debug-apply-evaluation", "--db", str(db_path), "--input", str(fixture_path)]) == 0
...
def test_status_summarizes_multiple_targets(tmp_path, capsys):
    ...
    assert main(["debug-apply-evaluation", "--db", str(db_path), "--input", str(default_fixture)]) == 0
    capsys.readouterr()
    assert main(["debug-apply-evaluation", "--db", str(db_path), "--input", str(target_1d_fixture)]) == 0
    capsys.readouterr()
