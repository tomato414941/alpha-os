...
def test_crypto_regime_momentum_signal_requires_trend_confirmation_and_funding_filter():
    ...
    assert signal.loc["2026-01-30"] == pytest.approx(0.0)
    assert signal.loc["2026-01-31"] == pytest.approx(1.0)
...
def test_create_frozen_evaluation_task_materializes_replay_case(tmp_path, capsys):
    ...
    try:
        ...
        store.upsert_evaluation_task(
            task=EvaluationTask(
                evaluation_task_id="case_source",
                strategy_id="strategy:source",
                evaluation_spec_id="protocol_long",
                **_evaluation_policy_parts(
                    sizing_method="equal_weight",
                ),
            )
    ...
    try:
        store.ensure_schema()
        strategy_state = store.get_trading_strategy("strategy:source")
        assert strategy_state is not None
        case_state = store.list_evaluation_tasks(
            strategy_id="strategy:source",
            evaluation_spec_id="protocol_long",
            limit=1,
        )[0]
