...
class BroadAlphaFillAuditOutcome:
    ticket_id: str
    opened_at: str
    checked_at: str
    horizon: str
...
def build_broad_alpha_fill_audit_outcomes(
    *,
    tickets_path: Path = ROOT / "current_broad_alpha_fill_audit_tickets.csv",
    now: datetime | None = None,
) -> tuple[BroadAlphaFillAuditOutcome, ...]:
    checked_at = now or datetime.now(UTC)
    rows = []
    for ticket in _read_rows(tickets_path):
        for horizon in _horizons(ticket.get("audit_horizons", "")):
            rows.append(_outcome_for_ticket(ticket=ticket, horizon=horizon, checked_at=checked_at))
    return tuple(rows)


def write_broad_alpha_fill_audit_outcomes_csv(
    rows: tuple[BroadAlphaFillAuditOutcome, ...],
    *,
    output_path: Path,
) -> Path:
...
def write_broad_alpha_fill_audit_outcomes_md(
    rows: tuple[BroadAlphaFillAuditOutcome, ...],
    *,
    output_path: Path,
) -> Path:
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        handle.write("# Current Broad Alpha Fill Audit Outcomes\n\n")
        handle.write(
            "These outcomes check fresh broad paper fill-audit tickets against public 1m candle path. "
            "They do not prove live fill quality.\n\n"
        )
        handle.write("| ticket | horizon | status | outcome | asset | venue | side | close | adverse | stop | candles | next step |\n")
        handle.write("| --- | --- | --- | --- | --- | --- | --- | ---: | ---: | --- | ---: | --- |\n")
        for row in rows:
            handle.write(
                "| "
...
def _outcome_for_ticket(
    *,
    ...
    candles = tuple(
        candle
        for candle in _fetch_candles(ticket=ticket, start=opened_at, end=end_at)
        if candle.start >= opened_at and candle.end <= end_at
    )
    if not candles:
    ...
    return BroadAlphaFillAuditOutcome(
        ...
        stop_bps=stop_bps,
        first_candle_at=candles[0].start.isoformat(timespec="seconds"),
        last_candle_at=candles[-1].end.isoformat(timespec="seconds"),
        ...
        evidence=(
            f"candles={len(candles)}; first={candles[0].start.isoformat(timespec='seconds')}; "
            f"last={candles[-1].end.isoformat(timespec='seconds')}; entry={ticket.get('entry_mark', '')}"
        ),
        next_step=_next_step(ticket=ticket, horizon_name=horizon_name, outcome=outcome, stop_status=stop_status),
    )


def _fetch_candles(*, ticket: dict[str, str], start: datetime, end: datetime) -> tuple[Candle, ...]:
    if ticket.get("venue") == "OKX":
        return _fetch_okx_candles(asset=ticket.get("asset", ""))
    return _fetch_hl_candles(asset=ticket.get("asset", ""), start=start, end=end)


def _fetch_hl_candles(*, asset: str, start: datetime, end: datetime) -> tuple[Candle, ...]:
    try:
    ...
    return tuple(
        Candle(
            start=_datetime_from_ms(candle.get("t")),
            end=_datetime_from_ms(candle.get("T")),
...
def _fetch_okx_candles(*, asset: str) -> tuple[Candle, ...]:
    try:
    ...
    rows = tuple(
        Candle(
            start=_datetime_from_ms(row[0]),
            end=_datetime_from_ms(float(row[0]) + 60_000.0),
            high=_float(row[2]),
            low=_float(row[3]),
            close=_float(row[4]),
        )
    ...
    return tuple(sorted(rows, key=lambda candle: candle.start))


def _path_metrics(*, candles: tuple[Candle, ...], entry: float, decision: str) -> PathMetrics:
...
def _next_step(*, ticket: dict[str, str], horizon_name: str, outcome: str, stop_status: str) -> str:
    ...
    if outcome == "paper_fill_audit_win":
        return f"compare {asset} {side} fill audit against prior broad paper path before promotion"
    if stop_status == "stop_triggered":
...
def main() -> None:
    ...
    parser.add_argument("--tickets-path", type=Path, default=ROOT / "current_broad_alpha_fill_audit_tickets.csv")
    parser.add_argument("--output-path", type=Path, default=ROOT / "current_broad_alpha_fill_audit_outcomes.csv")
    parser.add_argument("--md-output-path", type=Path, default=ROOT / "current_broad_alpha_fill_audit_outcomes.md")
    args = parser.parse_args()

    rows = build_broad_alpha_fill_audit_outcomes(tickets_path=args.tickets_path)
    write_broad_alpha_fill_audit_outcomes_csv(rows, output_path=args.output_path)
    write_broad_alpha_fill_audit_outcomes_md(rows, output_path=args.md_output_path)
    for row in rows:
