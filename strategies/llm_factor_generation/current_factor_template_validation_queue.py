...
ROUTE_RULES = {
    "ofi_liquidity_state_factor": RouteRule(
        ...
        validation_route="LOB maker-fill survival after book-depth policy replay",
        artifact_path=ROOT / "event_flow" / "current_lob_maker_fill_survival.csv",
        status_columns=("survival_status",),
        score_columns=("survival_score", "all_state_reward_bps", "filled_mark_reward_bps"),
        evidence_columns=(
            "state_family",
            "source_probe",
            "feature",
        ...
        next_step="do not promote optimistic maker rows until real queue, cancel, partial-fill, and adverse-selection labels pass",
    ),
    "crowded_positioning_unwind_factor": RouteRule(
        template_id="crowded_positioning_unwind_factor",
        validation_route="perp positioning unwind label gate before cross-venue context promotion",
        artifact_path=ROOT / "perp_market_map" / "current_crowding_unwind_label_gate.csv",
        status_columns=("label_gate_status",),
        score_columns=("label_gate_score",),
        evidence_columns=(
            "asset",
            "action",
        ...
        next_step="do not promote crowding unwind until forward labels are positive after funding and impact proxy",
    ),
    "source_diverse_news_shock_factor": RouteRule(
        template_id="source_diverse_news_shock_factor",
        validation_route="news event survival after source-independence gate",
        artifact_path=ROOT / "news_social" / "current_news_event_survival.csv",
        status_columns=("survival_status",),
        score_columns=("survival_score", "mean_directional_4h_bps", "mean_directional_1h_bps"),
        evidence_columns=(
            "symbol",
            "event_kind",
            "side",
        ...
        next_step="do not promote single-source news labels; require independent story support plus beta and cost checks",
    ),
    "prediction_market_crypto_beta_factor": RouteRule(
        ...
        artifact_path=ROOT / "prediction_markets" / "current_event_crypto_hedge_event_alignment.csv",
        status_columns=("alignment_status",),
        score_columns=("asset_directional_return_bps", "basket_directional_return_bps", "event_mark_return_bps"),
        evidence_columns=(
            "asset",
            "market_id",
