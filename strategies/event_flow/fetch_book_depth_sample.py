...
def fetch_book_depth_sample(
    *,
    ...
    for symbol in symbols:
        snapshots: list[BookDepthLiquiditySnapshot] = []
        for day in _days(start_date, end_date):
            snapshots.extend(_fetch_daily_book_depth_liquidity(symbol, day))
        path = output_dir / f"{symbol}.csv"
        _write_snapshots(tuple(snapshots), output_path=path)
        paths.append(path)
    return tuple(paths)
