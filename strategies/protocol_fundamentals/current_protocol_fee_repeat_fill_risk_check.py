...
class ProtocolFeeRepeatFillRiskCheck:
    ticket_id: str
    token_symbol: str
    protocol: str
    decision: str
...
def build_protocol_fee_repeat_fill_risk_checks(
    *,
    outcomes_path: Path = ROOT / "current_protocol_fee_repeat_outcomes.csv",
    execution_context_path: Path = ROOT / "current_protocol_fee_execution_context.csv",
    tickets_path: Path = ROOT / "current_protocol_fee_repeat_tickets.csv",
    default_notional_usd: float = 1_000.0,
    taker_fee_bps_per_fill: float = 4.0,
) -> tuple[ProtocolFeeRepeatFillRiskCheck, ...]:
    contexts = {
        (row.get("token_symbol", ""), row.get("protocol", "")): row
        for row in _read_rows(execution_context_path)
    }
    tickets = {
        _protocol_fee_key(row): row
        for row in _read_rows(tickets_path)
    }
    rows = tuple(
        _build_check(
            outcome=row,
            context=contexts.get(_protocol_fee_key(row), {}),
            ticket=tickets.get(_protocol_fee_key(row), {}),
...
def write_protocol_fee_repeat_fill_risk_checks_csv(
    rows: tuple[ProtocolFeeRepeatFillRiskCheck, ...],
    *,
    output_path: Path,
) -> Path:
    ...
    with output_path.open("w", newline="", encoding="utf-8") as handle:
        ...
        writer.writerow(tuple(ProtocolFeeRepeatFillRiskCheck.__dataclass_fields__))
        for row in rows:
            writer.writerow(
                (
                    row.ticket_id,
...
def write_protocol_fee_repeat_fill_risk_checks_md(
    rows: tuple[ProtocolFeeRepeatFillRiskCheck, ...],
    *,
    output_path: Path,
) -> Path:
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        handle.write("# Current Protocol Fee Repeat Fill Risk Check\n\n")
        handle.write(
            "This checks protocol-fee repeat mark wins against rough Hyperliquid spread, "
            "taker fee, funding, visible depth, and 1m candle path risk. It is not a live fill report.\n\n"
        )
        handle.write(
            "| ticket | subject | dir bps | cost bps | funding bps | net bps | depth 10bps | "
            "usage | MAE | MFE | stop50 | action | reason |\n"
        )
        handle.write("| --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: | ---: | --- | --- | --- |\n")
        for row in rows:
            handle.write(
                f"| {row.ticket_id} | {row.token_symbol}/{row.protocol} | "
                f"{row.directional_return_bps:.2f} | {row.round_trip_cost_bps:.2f} | "
                f"{row.estimated_funding_bps:.2f} | {row.estimated_net_after_cost_bps:.2f} | "
...
def _build_check(
    *,
...
) -> ProtocolFeeRepeatFillRiskCheck:
    token, protocol = _protocol_fee_key(outcome)
    decision = outcome.get("decision", "")
    directional_bps = _float(outcome.get("directional_return_bps"))
    spread_bps = _float(context.get("hl_spread_bps"))
    annualized_funding = _float(context.get("hl_annualized_funding"))
    funding_bps = _funding_bps(
        annualized_funding=annualized_funding,
        decision=decision,
        elapsed_minutes=_float(outcome.get("elapsed_minutes")),
    )
    notional = _float(ticket.get("candidate_size_usd")) or default_notional_usd
    depth = _float(context.get("hl_near_depth_10bps_notional"))
    usage = notional / depth if depth > 0.0 else 1_000_000.0
    ...
    return ProtocolFeeRepeatFillRiskCheck(
    ...
    *,
    ...
    if not context:
        return (
            ...
            f"refresh {subject} execution context before treating this as deployable",
        )
    ...
    if net_bps <= 0.0:
        return (
            ...
            f"do not promote {subject} unless a fresh trigger survives costs",
        )
    if max_adverse_excursion_bps <= -50.0:
        return (
            "stop_risk_blocks_repeat",
            "mark edge survived cost but would have breached a rough 50bps adverse-excursion stop",
            f"repeat {subject} only with explicit stop sizing or better entry timing",
        )
    return (
        "cost_adjusted_repeat_survived",
        "repeat mark edge survives rough cost, funding, visible-depth, and 50bps adverse-excursion checks",
        f"repeat {subject} with fresh protocol-fee evidence and record the same risk fields",
    )


def _protocol_fee_key(row: dict[str, str]) -> tuple[str, str]:
    opportunity = row.get("opportunity", "").removeprefix("protocol_fee_repeat:")
    token, _, protocol = opportunity.partition("/")
    if token and protocol:
        return token, protocol
    return row.get("token_symbol", "") or row.get("asset", ""), row.get("protocol", "")
...
def main() -> None:
    ...
    parser.add_argument("--outcomes-path", type=Path, default=ROOT / "current_protocol_fee_repeat_outcomes.csv")
    parser.add_argument(
        "--execution-context-path",
        type=Path,
        default=ROOT / "current_protocol_fee_execution_context.csv",
    )
    parser.add_argument("--tickets-path", type=Path, default=ROOT / "current_protocol_fee_repeat_tickets.csv")
    parser.add_argument("--default-notional-usd", type=float, default=1_000.0)
    parser.add_argument(
        "--output-path",
        type=Path,
        default=ROOT / "current_protocol_fee_repeat_fill_risk_check.csv",
    )
    parser.add_argument(
        "--md-output-path",
        type=Path,
        default=ROOT / "current_protocol_fee_repeat_fill_risk_check.md",
    )
    args = parser.parse_args()

    rows = build_protocol_fee_repeat_fill_risk_checks(
    ...
    write_protocol_fee_repeat_fill_risk_checks_csv(rows, output_path=args.output_path)
    write_protocol_fee_repeat_fill_risk_checks_md(rows, output_path=args.md_output_path)
    for row in rows:
        print(row.risk_action, row.token_symbol, f"net={row.estimated_net_after_cost_bps:.4f}")
