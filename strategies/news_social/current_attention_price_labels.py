...
def build_attention_price_label_rows(
    *,
    history_path: Path = ROOT / "attention_price_observation_history.csv",
) -> tuple[AttentionPriceLabelRow, ...]:
...
def write_attention_price_labels_csv(
    rows: tuple[AttentionPriceLabelRow, ...],
    *,
    output_path: Path,
) -> Path:
...
def write_attention_price_labels_md(
    rows: tuple[AttentionPriceLabelRow, ...],
    *,
    output_path: Path,
    top: int = 50,
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        handle.write("# Attention Price Forward Labels\n\n")
        handle.write(
            "This labels stored attention-price observations. Positive directional return means the "
            "observation direction was right before fees, spread, funding PnL, and slippage.\n\n"
        )
        handle.write(f"- total rows: `{len(rows)}`\n")
        handle.write(f"- labeled 1h rows: `{len(labeled_1h)}`\n\n")
        handle.write("| observed at | symbol | status | dir | priority | dir 1h | dir 4h | dir 12h | dir 24h | label status |\n")
        handle.write("| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | --- |\n")
        for row in rows[:top]:
            handle.write(
                f"| {row.observed_at} | {row.symbol} | {row.status} | {row.direction} | "
                f"{row.priority:.4f} | {_format_optional(row.directional_return_1h)} | "
...
def _sort_key(row: AttentionPriceLabelRow) -> tuple[bool, float, float]:
    return (
...
def main() -> None:
    ...
    parser.add_argument("--output-path", type=Path, default=ROOT / "current_attention_price_labels.csv")
    parser.add_argument("--markdown-output-path", type=Path, default=ROOT / "current_attention_price_labels.md")
    parser.add_argument("--top", type=int, default=50)
    args = parser.parse_args()

    rows = build_attention_price_label_rows(history_path=args.history_path)
    write_attention_price_labels_csv(rows, output_path=args.output_path)
    write_attention_price_labels_md(rows, output_path=args.markdown_output_path, top=args.top)
