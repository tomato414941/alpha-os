...
class FollowupRepeatForwardLabel:
    timestamp: str
    asset: str
    source: str
    source_action: str
...
def build_followup_repeat_forward_labels(
    *,
    input_path: Path = ROOT / "current_followup_repeat_observations.csv",
) -> tuple[FollowupRepeatForwardLabel, ...]:
    observations = tuple(
        row for row in _read_rows(input_path) if row.get("observation_status") == "ready_for_label"
    )
    candles_by_asset = {
        asset: _fetch_hyperliquid_candles(asset)
        for asset in sorted({row["asset"] for row in observations})
    }
    rows = tuple(
...
def write_followup_repeat_forward_labels(
    rows: tuple[FollowupRepeatForwardLabel, ...],
    *,
    output_path: Path,
) -> Path:
...
def write_followup_repeat_forward_labels_md(
    rows: tuple[FollowupRepeatForwardLabel, ...],
    *,
    output_path: Path,
    top: int = 40,
    ...
    with output_path.open("w", encoding="utf-8") as handle:
        handle.write("# Current Follow-Up Repeat Forward Labels\n\n")
        handle.write(
            "This labels source-specific follow-up observations against subsequent "
            "Hyperliquid returns. It is a repeat-observation label, not a PnL model.\n\n"
        )
        handle.write(
        ...
        handle.write("\n## Interpretation\n\n")
        handle.write(
            "`pending_15m` or `pending_1h` means the observation has not matured. "
            "Positive directional return means the source-specific direction was "
            "right over that horizon before fees, funding PnL, and slippage.\n"
        )
    return output_path
...
def _build_label(
    *,
    ...
    return FollowupRepeatForwardLabel(
...
def _sort_key(row: FollowupRepeatForwardLabel) -> tuple[bool, float, float]:
    return (
...
def main() -> None:
    ...
    parser.add_argument(
        "--output-path",
        type=Path,
        default=ROOT / "current_followup_repeat_forward_labels.csv",
    )
    parser.add_argument(
        "--md-output-path",
        type=Path,
        default=ROOT / "current_followup_repeat_forward_labels.md",
    )
    parser.add_argument("--top", type=int, default=40)
    args = parser.parse_args()

    rows = build_followup_repeat_forward_labels(input_path=args.input_path)
    write_followup_repeat_forward_labels(rows, output_path=args.output_path)
    write_followup_repeat_forward_labels_md(rows, output_path=args.md_output_path, top=args.top)
