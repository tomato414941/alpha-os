...
_HANDCRAFTED_HYPOTHESIS_SETS: dict[str, dict[str, list[str]]] = {
    "BTC": {
        "baseline": [
            "(roc_20 btc_ohlcv)",
...
def list_handcrafted_sets(asset: str) -> list[str]:
    return sorted(_HANDCRAFTED_HYPOTHESIS_SETS.get(asset.upper(), {}))


def get_handcrafted_expressions(asset: str, alpha_set: str) -> list[str]:
    sets = _HANDCRAFTED_HYPOTHESIS_SETS.get(asset.upper())
    if sets is None:
