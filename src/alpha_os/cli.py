...
from .cli_output import (
    ...
    print_hypothesis_competition_summary,
    print_hypothesis_details,
    print_hypothesis_metric,
...
def cmd_register_hypothesis(args: argparse.Namespace) -> int:
    ...
    print_hypothesis_details(hypothesis)
    return 0
...
def cmd_update_state(args: argparse.Namespace) -> int:
    ...
    print_hypothesis_metric(metric)
    return 0
...
def _apply_evaluation_inputs(
    ...
    *,
    ...
    if latest_snapshot is not None:
        ...
        print_hypothesis_metric(latest_metric)
    return 0
...
def cmd_show_portfolio_decisions(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        store.close()
    print(f"  DB:       {Path(cfg.db_path)}")
    print_portfolio_decisions(decisions)
    return 0
...
def cmd_show_validation(args: argparse.Namespace) -> int:
    ...
    print_validation_results(run, hypothesis_results, meta_results, decision_results)
    return 0
...
def cmd_summarize_validation(args: argparse.Namespace) -> int:
    with _runtime_store(args.db) as (_cfg, store):
        ...
        meta_results = store.list_validation_meta_results(run_id=run.run_id)
        decision_results = store.list_validation_decision_results(run_id=run.run_id)
    print_validation_summary(run, hypothesis_results, meta_results, decision_results)
    return 0
