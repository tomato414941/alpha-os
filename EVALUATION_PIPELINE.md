...
```
1. Evaluate:  expression → multi-horizon IC + per-asset breakdown
2. Submit:    IC > 0 → admission queue (source tagged)
```

This is the universal entry point. Human, GP, ML — all go through the
same evaluation and the same admission gate. No source gets special
treatment or bypasses validation.

Sources are tagged for observability, not for differential treatment:
- `source="gp"` — generated by DSL/GP evolutionary search
- `source="ml"` — generated by ML model
- `source="human"` — manually crafted

The evaluate step is optional but recommended: it lets the submitter
see the IC before committing to admission. Automated generators
(GP, ML) evaluate as part of their generation loop. Human submitters
...
- Inspect internals. The pipeline never looks at the expression
  structure, feature usage, or model architecture.
