...
- signals answer how predictions are produced
...
- `subject_set` identifies which subjects are considered together in one
  portfolio decision
...
A `subject_set` is not just a screenable universe. It is the explicit set of
subjects that one decision step is allowed to allocate across.
...
This makes the portfolio layer explicit about:

- which subjects are jointly optimized
- which runtime observation series back those subjects
- which pairwise dependence relations can exist in that decision step
...
It is not the same thing as a signal. A signal is one producer. A
...
This is why predictive quality alone is insufficient. A good signal or a
...
- lower influence for signals with small or fragile samples
...
- target-level scoring for signals
